package CRMLtoModelica
  package Types
    ...
    constant Integer buffer_size =50;
...
CRMLtoModelica.Types.Boolean4 out(start = CRMLtoModelica.Types.Boolean4.false4);
   ...
   CRMLtoModelica.Types.Boolean4 b " event trigger" ;
    
    Real t "time when even is registered";
    ...
    record CRMLPeriod
      ...
      Boolean isLeftBoundaryIncluded (start = false) "If true, the left boundaries of the time periods are included";
      Boolean isRightBoundaryIncluded (start = false) "If true, the right boundaries of the time periods are included";
    ...
    record CRMLPeriods
      ...
      //outer parameter Integer buffer_size=50;
     CRMLPeriod  period [50] ;
...
equation
P.is_open = if( (P.start_event.b == Boolean4.true4) and not (P.close_event.b == Boolean4.true4))then true else false;
for i in 1: size(P.period, 1) loop
   // P.period[i].start_event.b = P.start_event.ticks[i];
    //P.period[i].close_event.b = P.close_event.ticks[i];
   end for;

algorithm
  
  //e := (clock.b == CRMLtoModelica.Types.Boolean4.true4);
  when (P.start_event.b  == CRMLtoModelica.Types.Boolean4.true4 and change(P.start_event.b)) then
   
    P.period[P.counter].is_open := true;
     end when;
   when (P.close_event.b  == CRMLtoModelica.Types.Boolean4.true4 and change(P.close_event.b)) then
    
    P.period[P.counter].is_open :=  false;
    P.counter := pre(P.counter)+1;
    end when;
